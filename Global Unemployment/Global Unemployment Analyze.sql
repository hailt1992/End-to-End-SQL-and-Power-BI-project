...
-----Join global_data and mapping_list by iso_code and alpha_3 field
...
SELECT gd.country,
		...
		gd.age,
		gd."year",
		ROUND((gd.unemployment_rate/100) :: DECIMAL(10,2),2) AS unemployment_rate,
		ROUND((gd.employment_rate/100) :: DECIMAL (10,2),2) AS employment_rate
FROM global_data gd
LEFT JOIN mapping_list ml ON gd.iso_code = ml.alpha_3;
...
FROM female f
FULL OUTER JOIN male m ON f.country = m.country
...
--Which countries have experienced the highest volatility (frequent spikes and drops) in unemployment rates over the last 34 years? 
WITH cte_1 AS (
			SELECT country, 
